...
injection.compile(optimizer=keras.optimizers.Adam(lr=0.00001), loss=[sum_squared_error, 'categorical_crossentropy'], loss_weights=[1.2, 0.7])
...
plot_model(injector, to_file='imgs/models/autoencoder/cifar_injector34.png', show_shapes=True)
plot_model(deinjector, to_file='imgs/models/autoencoder/cifar_not_injector34.png', show_shapes=True)
plot_model(injection, to_file='imgs/models/autoencoder/cifar_injection34.png', show_shapes=True)
plot_model(not_injection, to_file='imgs/models/autoencoder/cifar_not_injection34.png', show_shapes=True)

epoch_number = 4 * 60
batches_number = 1000
batch_size = 50

f1=open('neuro-example/autoencoder/cifar-34/stats.txt', 'a', 1)
...
for e in range(epoch_number):
    ...
    injector.save('neuro-example/autoencoder/cifar-34/cifar_injector34-' + str(e) + '.h5')
    deinjector.save('neuro-example/autoencoder/cifar-34/cifar_deinjector34-' + str(e) + '.h5')
    injection.save('neuro-example/autoencoder/cifar-34/cifar_injection34-' + str(e) + '.h5')
    not_injection.save('neuro-example/autoencoder/cifar-34/cifar_not_injection34-' + str(e) + '.h5')
