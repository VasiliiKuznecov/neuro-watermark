...
injection.compile(optimizer=keras.optimizers.Adam(lr=0.0005), loss=[sum_squared_error, 'categorical_crossentropy'], loss_weights=[1, 0.7])
not_injection.compile(optimizer=keras.optimizers.Adam(lr=0.0005), loss='categorical_crossentropy', loss_weights=[0.7])

injection.summary()
not_injection.summary()


plot_model(injector, to_file='imgs/models/autoencoder/cifar_injector20.png', show_shapes=True)
plot_model(deinjector, to_file='imgs/models/autoencoder/cifar_not_injector20.png', show_shapes=True)
plot_model(injection, to_file='imgs/models/autoencoder/cifar_injection20.png', show_shapes=True)
plot_model(not_injection, to_file='imgs/models/autoencoder/cifar_not_injection20.png', show_shapes=True)

epoch_number = 60
batches_number = 1000
batch_size = 50

f1=open('neuro-example/autoencoder/cifar-20/stats.txt', 'w+')
...
for e in range(epoch_number):
    ...
    injector.save('neuro-example/autoencoder/cifar-20/cifar_injector20-' + str(e) + '.h5')
    deinjector.save('neuro-example/autoencoder/cifar-20/cifar_deinjector20-' + str(e) + '.h5')
    injection.save('neuro-example/autoencoder/cifar-20/cifar_injection20-' + str(e) + '.h5')
    not_injection.save('neuro-example/autoencoder/cifar-20/cifar_not_injection20-' + str(e) + '.h5')
